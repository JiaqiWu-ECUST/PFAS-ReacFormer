...
def reactants_desc_vec(
    ...
    assert ext_feat_gen is not None, "ext_feat_gen 未找到，请确认 descriptors.py 在 PYTHONPATH 中"
...
class PFASDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        required_cols = ["substrate_smiles", "product_smiles"]
        ...
        candidate_cond_cols = [
            ...
            "organic matter content（％）", "cation exchange capacity（cmol/kg）", "TP(mg/g)",
            "TN(mg/g)", "TOC(mg/g)", "moisture content(％)", "TTS(g/L)", "sludge_volume(mL)",
            "DO(mg/L)", "reaction time(d)",
        ]
        ...
        if len(cond_cols) > 0:
            pass
            cond_mat = df[cond_cols].to_numpy(dtype=np.float32)
        else:
            pass
            cond_mat = None
        ...
        for idx, row in tqdm(df.iterrows(), total=len(df), desc="Building PFASDataset"):
            ...
            if self.use_desc:
                rct_smiles = row["substrate_smiles"]
        ...
        if self.use_desc and len(desc_list) > 0:
            ...
            torch.save(desc_tensor, "bio_pfas_rdkit_desc_tensor.pt")
            pass
    ...
    def row_to_data(row, system_id=None):
        ...
        smi_r = str(row["substrate_smiles"])
        ...
        d.product_smiles = str(row["product_smiles"])

        for col in ["pathway_name", "pathway_id", "reaction_id"]:
            if col in row.index:
                setattr(d, col, row[col])
...
def main():
    ...
    ap.add_argument("--csv", required=True, help="包含 substrate_smiles, product_smiles 的 CSV/TSV 文件")
    ap.add_argument("--root", default=".", help="PyG 数据缓存目录")
    ap.add_argument("--use_desc", action="store_true", help="是否计算分子描述符 mol_desc")
    ap.add_argument("--ext_feat_type", default="rdkit", help="描述符类型，如 'morgan'")
    ...
    if args.out:
        torch.save((ds.data, ds.slices), args.out)
        pass
